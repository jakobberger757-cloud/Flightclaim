...
async def _send_result_email(email: str, result: dict, refund: float, you_keep: float):
    ...
    app_url = os.environ.get("FRONTEND_URL", "https://flightclaim.com")

    html = f"""
    <div style="font-family:sans-serif;max-width:560px;margin:0 auto;padding:32px;background:#fff">
      <h1 style="font-size:24px;font-weight:800;margin-bottom:4px">You're owed ${refund:.0f}</h1>
      ...
      <p style="color:#999;font-size:12px;text-align:center;margin-top:16px">
        No win, no fee. Airlines must respond within 7 business days by law.
      </p>
    </div>"""
    ...
    if sendgrid_key:
            ...
            async with httpx.AsyncClient() as client:
                await client.post(
                    ...
                    json={
                        ...
                        "from": {"email": os.environ.get("FROM_EMAIL", "claims@flightclaim.com"), "name": "FlightClaim"},
                        "subject": f"Your {airline} refund: ${refund:.0f} owed",
        ...
        print(f"EMAIL (no SendGrid key): would send ${refund} result to {email}. Add SENDGRID_API_KEY to Railway.")
