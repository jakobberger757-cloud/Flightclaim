...
RADAR_PROMPT = """You are an expert Reddit community strategist helping FlightClaim identify high-value posts to engage with.

FlightClaim: Users paste airline cancellation/delay emails → AI checks DOT refund eligibility → 20% fee only if money recovered → flightclaim.today

Your job: analyze this Reddit post and return a structured JSON triage + reply drafts.

SCORING CRITERIA (1-10):
Boost score for:
- Recent cancellation (still happening or just happened)
- Mentions voucher instead of cash refund
- Confusion about rights / what they're owed
- Asking "what do I do" or "am I owed anything"
- Flight was cancelled (not just delayed)
- US-based airline (United, Delta, American, Southwest, etc)
- Auto-rebooked without consent
- Explicit refund mention

Lower score for:
- Just venting, no question
- Already resolved / accepted voucher happily
- Non-US airline or international only
- Old post (no recency signal)
- Asking about something unrelated to refunds
- Situation clearly ineligible

MOD RISK:
- Low: user is asking for help, educational reply fits naturally
- Medium: user is venting, a link could look promotional
- High: strict subreddit, any mention of a tool will get flagged

REPLY TONE RULES (critical):
- Sound like a real person who travels a lot and learned this the hard way
- Lead with empathy / validation always
- Provide real value before any mention of a tool
- Never say "I built", "AI-powered", "startup", "solution", "platform"
- Never sound like customer support or a founder
- Natural Reddit voice — slightly frustrated with airlines
- Good pattern: "yeah this is BS / I dealt with this / here's what I learned / here's something that helped me"
- Draft 1: pure value, no tool mention, no link
- Draft 2: soft mention, sounds like personal discovery ("I actually used a free checker someone posted here")
- Draft 3: link ONLY if it would feel natural and earned — if not, explain why not and give a safer version instead
...
{{
  "score": <1-10>,
  "recommended_action": "<Reply now | Reply now no link | Wait for reply | Skip>",
  "why": "<2-3 sentences on key signals>",
  "mod_risk": "<Low | Medium | High>",
  "mod_risk_reason": "<one sentence>",
  "conversion_likelihood": "<Low | Medium | High>",
  "conversion_reason": "<one sentence>",
  "best_next_move": "<Reply with value only | Reply with soft mention | Wait for reply before mentioning tool | Link is okay here | Do not engage>",
  "stress_test": {{
    "spammy_risk": "<what could make a reply feel spammy here>",
    "link_hurts_trust": <true|false>,
    "better_as_value_only": <true|false>,
    "op_likely_to_respond": <true|false>,
    "enough_context": <true|false>,
    "link_earned_or_unearned": "<earned | unearned | borderline>",
    "non_us_risk": "<Low | Medium | High>"
  }},
  "reply_1": {{
    "style": "Value only — no mention",
    "text": "<reply text, Reddit-native, no link, pure value>"
  }},
  "reply_2": {{
    "style": "Soft mention — sounds personal",
    "text": "<reply text, natural soft mention of a checker, no hard pitch>"
  }},
  "reply_3": {{
    "style": "Link if earned — or safer alternative",
    "text": "<reply text with flightclaim.today if earned, OR safer version with explanation if not>",
    "link_included": <true|false>,
    "reason": "<why link is or isn't included>"
  }},
  "checklist": {{
...
async def radar_analyze(request: RadarAnalyzeRequest):
    """Internal operator endpoint — Reddit opportunity triage."""
    secret = os.environ.get("OPERATOR_SECRET", "changeme")
    if request.operator_key != secret:
        raise HTTPException(403, "Unauthorized")

    if not request.post or len(request.post.strip()) < 20:
        raise HTTPException(400, "Post text too short to analyze.")
    ...
    try:
        response = client.messages.create(
            ...
            messages=[{
                "role": "user",
                "content": RADAR_PROMPT.format(post=request.post.strip())
            }]
        )
        raw = response.content[0].text.strip()
        if raw.startswith("```"):
        ...
        raise HTTPException(500, "Could not parse analysis result.")
    except Exception as e:
...
@app.get("/radar")
async def serve_radar(key: str = ""):
    secret = os.environ.get("OPERATOR_SECRET", "changeme")
    if key != secret:
        return HTMLResponse("<html><body style='font-family:sans-serif;padding:40px'>Unauthorized</body></html>", status_code=403)
    try:
        return FileResponse("radar.html")
    except Exception:
        return HTMLResponse("<html><body>radar.html not found</body></html>", status_code=404)
